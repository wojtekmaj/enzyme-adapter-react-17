...
export default function describeContainsAllMatchingElements({ Wrap }) {
  describe('.containsAllMatchingElements(nodes)', () => {
    it('throws TypeError if non-array passed in', () => {
      const wrapper = Wrap(<div>Hello</div>);

      expect(() => wrapper.containsAllMatchingElements(<div>Hello</div>)).to.throw(
        TypeError,
    ...
    it('matches on array of nodes that each look like rendered nodes, with nested elements', () => {
      ...
      expect(wrapper.containsAllMatchingElements([<p>Hello</p>, <p>Goodbye</p>])).to.equal(true);
    });
    ...
    it('matches on an array of nodes that all look like one of the rendered nodes', () => {
      ...
      expect(
        wrapper.containsAllMatchingElements([<div>Hello World</div>, <div>Goodbye World</div>]),
      ).to.equal(true);
      expect(
