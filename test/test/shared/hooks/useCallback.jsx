...
export default function describeUseCallback({ Wrap, isShallow }) {
  describe('hooks: useCallback', () => {
    ...
    function ComponentUsingCallbackHook({ onChange }) {
      ...
      return <RendersNull callback={callback} />;
    }
    ...
    itIf(
      !isShallow,
      'get same callback when using `useCallback` and rerender with same prop in dependencies',
      () => {
        const onChange = () => {};
    ...
    it('get different callback when using `useCallback` and rerender with different prop in dependencies', () => {
      const onChange = () => {};
      const wrapper = Wrap(
        <ComponentUsingCallbackHookWithRelatedProp onChange={onChange} relatedProp="456" />,
      );
      const initialCallback = wrapper.find(RendersNull).prop('callback');
